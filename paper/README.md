...
- **Zip for MDPI “Manuscript (Word/ZIP)” upload (main text only):** `Nexus-NRS_HALO40_manuscript_MDPI.zip` — contains `mdpi_proceedings_manuscript.tex`, **all six main-text figures** (`nexus_nrs_dashboard.png`, `wronskian_drift.pdf`, `dwuck4_compare.pdf`, `16Opd_dcs_paper.pdf`, `li11_pp_setV_setS.pdf`, `li11_pd_10li_resonance.pdf`), and `Definitions/` (class files required to compile). **No PDF, no supplementary files** inside the zip (upload supplementary materials separately if the journal asks). Regenerate (verify with a clean-room `pdflatex` build after):  
  `cd paper && zip -r Nexus-NRS_HALO40_manuscript_MDPI.zip mdpi_proceedings_manuscript.tex nexus_nrs_dashboard.png wronskian_drift.pdf dwuck4_compare.pdf 16Opd_dcs_paper.pdf li11_pp_setV_setS.pdf li11_pd_10li_resonance.pdf Definitions -x "*.DS_Store" -x "*/.DS_Store"`
