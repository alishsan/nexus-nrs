;; Copyright (c) generateme/fastmath (MIT). Vendored from 3.0.0-alpha4.
;; One edit: `si` was renamed to `sine-integral-shifted` + var alias — JVM class names
;; `special$Si` and `special$si` collide on case-insensitive filesystems (macOS APFS).
...
(defn sine-integral-shifted
  "Sine integral, Si shifted by -pi/2 (formerly `si`; renamed to avoid APFS class-name collision with `Si`)."
  ^double [^double x] (m/- (Si x) m/HALF_PI))

(def si sine-integral-shifted)
