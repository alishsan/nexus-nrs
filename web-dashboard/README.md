...
- **`GET /api/transfer-default?target=16O&reaction_type=d-p`** loads the default **¹⁶O(d,p)** ZR benchmark curve (**mb/sr**). **`reaction_type=p-d`** keeps the legacy **(p,d)** POST + global-optics default.
- **`POST /api/transfer`** with **`reaction_type=d-p`** runs the same **¹⁶O(d,p)** benchmark for any **`target`** in the JSON body (non-**16O** targets get the same curve; the response **`note`** says so). Lab energies are deuteron **E_lab**; mesh via **`transfer_h`**, **`transfer_r_max`**; **`transfer_L_max`** optional (default 6).
